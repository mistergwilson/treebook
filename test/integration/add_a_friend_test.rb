...
class AddAFriendTest < ActionDispatch::IntegrationTest
  ...
  test "that adding a friend works" do
  	sign_in_as users(:gregory), "testing"

  	get "/user_friendships/new?friend_id=#{users(:jim).user_name}"

  	assert_response :success

  	assert_difference 'UserFriendship.count' do 
  		post "/user_friendships", user_friendship: {friend_id: users(:jim).user_name}
