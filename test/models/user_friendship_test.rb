...
class UserFriendshipTest < ActiveSupport::TestCase
  ...
  test "that creating a friendship works without raising an exception" do
  	assert_nothing_raised do
  		UserFriendship.create user: users(:gregory), friend: users(:mike)
  	end
  end


  test "that creating a friendship based on user id and friend id works" do
  	UserFriendship.create user_id: users(:gregory).id, friend_id: users(:mike).id
  	assert users(:gregory).pending_friends.include?(users(:mike))
  end

  context "a new instance" do
    setup do
      @user_friendship = UserFriendship.new user: users(:gregory), friend: users(:mike)
    end
  ...
  context "#send_request_email" do
    setup do
      @user_friendship = UserFriendship.request user: users(:gregory), friend: users(:mike)
    end
...
context "mutual_friendship" do
    setup do
      UserFriendship.request users(:gregory), users(:mike)
      @friendship1 = users(:gregory).user_friendships.where(friend_id: users(:mike).id).first
      @friendship2 = users(:mike).user_friendships.where(friend_id: users(:gregory).id).first
    end
  ...
  context "#accept!" do
    setup do
      @user_friendship = UserFriendship.create user: users(:gregory), friend: users(:mike)
    end
